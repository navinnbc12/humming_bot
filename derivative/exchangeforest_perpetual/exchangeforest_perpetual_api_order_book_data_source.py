...
from hummingbot.connector.derivative.exchangeforest_perpetual.exchangeforest_perpetual_order_book import ExchangeforestPerpetualOrderBook
from hummingbot.connector.derivative.exchangeforest_perpetual.exchangeforest_perpetual_utils import convert_to_exchange_trading_pair
...
TICKER_PRICE_URL = "{}/api/bookTicker"
TICKER_PRICE_CHANGE_URL = '{}/api/NodeGetTickerPrice'
...
class ExchangeforestPerpetualAPIOrderBookDataSource(OrderBookTrackerDataSource):
    def __init__(self, trading_pairs: List[str] = None, domain: str = "exchangeforest_perpetual"):
        super().__init__(trading_pairs)
        self._order_book_create_function = lambda: OrderBook()
        self._base_url = TESTNET_BASE_URL if domain == "exchangeforest_perpetual_testnet" else PERPETUAL_BASE_URL
        self._stream_url = TESTNET_STREAM_URL if domain == "exchangeforest_perpetual_testnet" else DIFF_STREAM_URL
    ...
    async def get_last_traded_prices(cls, trading_pairs: List[str], domain=None) -> Dict[str, float]:
        logging.info('inside get_last_traded')
        tasks = [cls.get_last_traded_price(t_pair, domain) for t_pair in trading_pairs]
        results = await safe_gather(*tasks)
        aa = {t_pair: result for t_pair, result in zip(trading_pairs, results)}
        return aa
    @classmethod
    async def get_last_traded_price(cls, trading_pair: str, domain=None) -> float:
        async with aiohttp.ClientSession() as client:
            url = TESTNET_BASE_URL if domain == "exchangeforest_perpetual_testnet" else PERPETUAL_BASE_URL
            logging.info('inside get_last_traded_price ')
    ...
    async def fetch_trading_pairs(domain=None) -> List[str]:
        try:
            from hummingbot.connector.derivative.exchangeforest_perpetual.exchangeforest_perpetual_utils import convert_from_exchange_trading_pair
    ...
    async def get_snapshot(client: aiohttp.ClientSession, trading_pair: str, limit: int = 1000, domain=None) -> Dict[str, Any]:
        ...
        logging.info('ssss %s'%params)
        async with client.get(SNAPSHOT_REST_URL.format(domain), params=params) as response:
            response: aiohttp.ClientResponse = response
            if response.status != 200:
                raise IOError(f"Error fetching Exchangeforest market snapshot for {trading_pair}. "
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        async with aiohttp.ClientSession() as client:
            ...
            snapshot_msg: OrderBookMessage = ExchangeforestPerpetualOrderBook.snapshot_message_from_exchange(
                snapshot,
                snapshot_timestamp,
            ...
            try:
                ...
                async with websockets.connect(stream_url) as ws:
                    ...
                    async for raw_msg in self.ws_messages(ws):
                        ...
                        order_book_message: OrderBookMessage = ExchangeforestPerpetualOrderBook.diff_message_from_exchange(
                            msg_json,
                            timestamp
                        )
        ...
        while True:
            try:
                ...
                async with websockets.connect(stream_url) as ws:
                    ...
                    async for raw_msg in self.ws_messages(ws):
                        msg_json = ujson.loads(raw_msg)
                        trade_msg: OrderBookMessage = ExchangeforestPerpetualOrderBook.trade_message_from_exchange(msg_json)
    ...
    async def listen_for_order_book_snapshots(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        while True:
            try:
                # trading_pairs: List[str] = await self.get_trading_pairs()
                logging.info('inside booksnapshotss')
                async with aiohttp.ClientSession() as client:
                    for trading_pair in self._trading_pairs:
                        try:
