...
EXCHANGEFOREST_API_ENDPOINT = "https://api.exchangeforest.{}/api/v1/"
EXCHANGEFOREST_USER_STREAM_ENDPOINT = "userDataStream"
EXCHANGEFOREST_WSS_USER_STREAM = "wss://stream.exchangeforest.{}:9443/ws/"


class exchangeforestAPIUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    def __init__(self, exchangeforest_client: exchangeforestClient, domain: str = "com"):
    ...
    async def get_listen_key(self):
        async with aiohttp.ClientSession() as client:
            url = exchangeforest_API_ENDPOINT.format(self._domain)
            async with client.post(f"{url}{exchangeforest_USER_STREAM_ENDPOINT}",
                                   headers={"X-MBX-APIKEY": self._exchangeforest_client.API_KEY}) as response:
    ...
    async def ping_listen_key(self, listen_key: str) -> bool:
        async with aiohttp.ClientSession() as client:
            url = exchangeforest_API_ENDPOINT.format(self._domain)
            async with client.put(f"{url}{exchangeforest_USER_STREAM_ENDPOINT}",
                                  headers={"X-MBX-APIKEY": self._exchangeforest_client.API_KEY},
    ...
    async def get_ws_connection(self) -> websockets.WebSocketClientProtocol:
        url = exchangeforest_WSS_USER_STREAM.format(self._domain)
