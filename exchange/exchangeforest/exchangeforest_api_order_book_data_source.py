...
from hummingbot.connector.exchange.exchangeforest.exchangeforest_order_book import ExchangeforestOrderBook
from hummingbot.connector.exchange.exchangeforest.exchangeforest_utils import convert_to_exchange_trading_pair

TRADING_PAIR_FILTER = re.compile(r"(BTC|ETH|USDT)$")

SNAPSHOT_REST_URL = "https://devexplorer.exchangeforest.com.{}/api/Depth"
DIFF_STREAM_URL = "wss://stream.exchangeforest.{}:9443/ws"
TICKER_PRICE_CHANGE_URL = "https://devexplorer.exchangeforest.com.{}/api/NodeGetTickerPrice"
EXCHANGE_INFO_URL = 'https://devexplorer.exchangeforest.com.{}/api/v1/exchangeInfo'


class ExchangeforestAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_all_mid_prices(domain="com") -> Optional[Decimal]:
        from hummingbot.connector.exchange.exchangeforest.exchangeforest_utils import convert_from_exchange_trading_pair
        async with aiohttp.ClientSession() as client:
            url = "https://api.exchangeforest.{}/api/v3/ticker/bookTicker".format(domain)
    ...
    async def fetch_trading_pairs(domain="com") -> List[str]:
        try:
            ...
            # Do nothing if the request fails -- there will be no autocomplete for exchangeforest trading pairs
            pass
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        async with aiohttp.ClientSession() as client:
            ...
            snapshot_msg: OrderBookMessage = ExchangeforestOrderBook.snapshot_message_from_exchange(
                snapshot,
                snapshot_timestamp,
            ...
            try:
                ...
                async with websockets.connect(stream_url) as ws:
                    ...
                    async for raw_msg in self._inner_messages(ws):
                        ...
                        trade_msg: OrderBookMessage = ExchangeforestOrderBook.trade_message_from_exchange(msg)
        ...
        while True:
            try:
                ...
                async with websockets.connect(stream_url) as ws:
                    ...
                    async for raw_msg in self._inner_messages(ws):
                        msg = ujson.loads(raw_msg)
                        order_book_message: OrderBookMessage = ExchangeforestOrderBook.diff_message_from_exchange(
                            msg, time.time())
                ...
                async with aiohttp.ClientSession() as client:
                    for trading_pair in self._trading_pairs:
                        try:
                            ...
                            # Be careful not to go above exchangeforest's API rate limits.
